...
const Category = () => {
  ...
  // --- 2. PEGAR DADOS GLOBAIS E FUNÇÕES DO CARRINHO ---
  ...
  // --- 3. ATUALIZAR A LÓGICA DE FILTRO ---
  // Pega a lista de produtos base da categoria (agora do estado global)
  const baseProducts = useMemo(() => {
    return products.filter(p => p.category === id);
  }, [id, products]); // Re-calcula se o ID da URL ou a lista global de produtos mudar

  // A lógica de filtro e ordenação continua a mesma,
  // mas agora ela reage a mudanças no 'baseProducts'
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="ml-20 p-8">
        ...
        {/* Grid de Produtos (agora usa 'filteredAndSortedProducts' que vem do estado global) */}
          ...
          {filteredAndSortedProducts.length > 0 ? (
            filteredAndSortedProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="bg-card rounded-2xl border border-border shadow-elegant hover:shadow-float smooth-transition p-4 flex flex-col"
                >
                  <div
                  ...
                  >
                    <div className="relative aspect-square bg-muted rounded-xl mb-3 overflow-hidden">
                      ...
                      {stockBadge && (
                        <div
                          className={`absolute top-2 right-2 px-2 py-1 rounded-full text-xs font-bold ${stockBadge.class}`}
                        >
                          {stockBadge.text}
                        </div>
                      )}
      ...
      {/* Renderiza o Modal (sem alteração) */}
      {selectedProduct && (
        <ProductModal
